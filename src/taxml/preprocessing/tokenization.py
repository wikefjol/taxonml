...
class KmerStrategy:
    def __init__(self, k: int, padding_alphabet: Sequence[str] = ('A','C','G','T'), rng: Optional[random.Random] = None):
        if not isinstance(k, int) or k <= 0:
            raise ValueError(f"[PROCESSING.TOKENIZATION] 'KmerStrategy.__init__': invalid k={k}; must be int > 0")
        if not padding_alphabet:
            raise ValueError("padding_alphabet must be non-empty")
        self.k = k
        self.padding_alphabet = list(padding_alphabet)
    ...
    def __repr__(self):
        return (
            ...
            f"k={self.k}, "
            f"padding_alphabet_size={len(self.padding_alphabet)})"
        )
    ...
    def _make_divisible_by_k(self, input_seq: List[str]) -> List[str]:
        ...
        while len(seq) % self.k != 0:
            seq.append(self.rng.choice(self.padding_alphabet))
        return seq
